...
def str_to_sentences(x):
    ...
    sentences = [sent.rstrip().lstrip() for sent in sentences]
...
def is_section_name(last_sentence, sentence):
    if len(sentence.split(' ')) <= 3 and sentence.split(' ') != [''] and last_sentence == '':
            return True
    else:
            return False